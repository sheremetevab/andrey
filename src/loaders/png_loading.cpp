...
int _png_load(const char* file, int* width, int* height){
    ...
    *height = t_height;
    ...
    glGenTextures(1, &texture);  // создание текстуры
    glBindTexture(GL_TEXTURE_2D, texture);  // привязка текстуры
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, t_width, t_height, 0,
        alpha, GL_UNSIGNED_BYTE, (GLvoid *) image_data);  // передача данных в текстуру
//  установка параметров
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glBindTexture(GL_TEXTURE_2D, 0);  // привязка текстуры
...
Texture* load_texture(std::string filename) {
    int width, height;
    GLuint texture = _png_load(filename.c_str(), &width, &height);  // загружаем текстуру
    if (texture == 0) {
